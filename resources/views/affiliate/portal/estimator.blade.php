...
<x-affiliate-layout title="Earnings estimator" active="estimator">
    <div class="mb-5 rounded-xl border border-slate-200 bg-white p-5">
        ...
        <p class="mb-3 text-xs text-slate-500">Rough projection only — actual commission follows real payments and your approved referrals.</p>
            ...
            <div>
                ...
                @if ($plans->isNotEmpty())
                    <div class="mt-1 text-xs text-slate-500">
                        @foreach ($plans as $pl)
                            <a href="#" onclick="this.closest('form').price.value={{ (float) $pl->price_monthly }};return false" class="mr-2 text-orange-600">{{ $pl->name }} ${{ number_format((float) $pl->price_monthly, 2) }}</a>
                        @endforeach
                    </div>
                @endif
            </div>
            <div><label class="mb-1 block text-xs font-medium">Your commission rate (%)</label>
                <input name="rate" type="number" step="0.001" value="{{ $inputs['rate'] }}" class="w-full rounded-lg border-slate-300 text-sm"></div>
            <div><label class="mb-1 block text-xs font-medium">Businesses referred</label>
                <input name="count" type="number" value="{{ $inputs['count'] }}" class="w-full rounded-lg border-slate-300 text-sm"></div>
            <div><label class="mb-1 block text-xs font-medium">Avg. months each stays</label>
                <input name="months" type="number" value="{{ $inputs['months'] }}" class="w-full rounded-lg border-slate-300 text-sm"></div>
            <div><label class="mb-1 block text-xs font-medium">New sign-ups / month</label>
                <input name="new_per_month" type="number" value="{{ $inputs['newPerMonth'] }}" class="w-full rounded-lg border-slate-300 text-sm"></div>
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
            @foreach ([
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-4">
                    <div class="text-[11px] uppercase tracking-wide text-slate-400">{{ $l }}</div>
                    <div class="mt-1 text-xl font-bold {{ str_starts_with($sub, 'text-') ? $sub : '' }}">{{ $val }}</div>
                    @if ($sub && ! str_starts_with($sub, 'text-'))<div class="text-xs text-slate-400">{{ $sub }}</div>@endif
                </div>
            @endforeach
        </div>
    @endif

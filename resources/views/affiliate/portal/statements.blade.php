...
<x-affiliate-layout title="Statements" active="statements">
    ...
    <div class="mb-5 rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
        ...
        @if ($eligible->isEmpty())
            <p class="text-sm text-slate-400 dark:text-slate-500">No months are ready yet. Commission has to be approved first.</p>
        @else
                    ...
                    <tr class="border-t border-slate-100 dark:border-slate-800/60">
                        ...
                        <td class="text-right">
                            <a href="{{ route('affiliate.portal.statements', ['y' => $mn->period_year, 'm' => $mn->period_month]) }}"
                               class="rounded border border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white px-3 py-1 text-xs hover:bg-slate-50 dark:hover:bg-slate-700">Preview</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
    ...
    @if ($previewRows->isNotEmpty())
        <div class="mb-5 rounded-xl border border-orange-300 bg-white p-5 dark:border-orange-900/60 dark:bg-slate-900">
            ...
            <table class="w-full text-sm">
                <thead><tr class="text-left text-slate-500 dark:text-slate-400"><th class="py-1">Business</th><th>Type</th><th>Base</th><th>Rate</th><th>Amount</th></tr></thead>
                <tbody>
                @foreach ($previewRows as $r)
                    <tr class="border-t border-slate-100 dark:border-slate-800/60">
                        <td class="py-2">{{ $r->business?->name ?? $r->business_name_snapshot }}</td>
                        <td class="text-slate-500 dark:text-slate-400">{{ $r->kind }}</td>
            ...
            <form method="POST" action="{{ route('affiliate.portal.statements.finalize') }}" class="mt-3"
                ...
                <a href="{{ route('affiliate.portal.statements') }}" class="ml-2 text-sm text-slate-500 dark:text-slate-400 hover:underline">Cancel</a>
            </form>
        </div>
    @endif
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
        ...
        @if ($payouts->isEmpty())
            <p class="text-sm text-slate-400 dark:text-slate-500">None yet.</p>
        @else
            <table class="w-full text-sm">
                <thead><tr class="text-left text-slate-500 dark:text-slate-400"><th class="py-1">Number</th><th>Period</th><th>Amount</th><th>Status</th><th>Paid</th><th></th></tr></thead>
                <tbody>
                @foreach ($payouts as $p)
                    <tr class="border-t border-slate-100 dark:border-slate-800/60">
                        ...
                        <td class="text-slate-500 dark:text-slate-400">{{ $p->paid_at?->format('d/m/Y') ?? '—' }}</td>
                        <td class="text-right">
                            <a href="{{ route('affiliate.portal.statements.show', $p) }}"
                               class="rounded border border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white px-3 py-1 text-xs hover:bg-slate-50 dark:hover:bg-slate-700">Open</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>

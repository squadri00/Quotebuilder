...
<x-affiliate-layout title="Prospects & market" active="prospects">
        ...
        <form method="POST"
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                ...
                <div>
                    ...
                    <select name="country" class="w-full rounded-lg border-slate-300 text-sm">
                        <option value="">—</option>
                ...
                <div>
                    ...
                    <select name="estimated_plan_id" class="w-full rounded-lg border-slate-300 text-sm">
                        <option value="">—</option>
            ...
            <textarea name="notes" rows="3" class="w-full rounded-lg border-slate-300 text-sm">{{ $v('notes') }}</textarea>
            <div class="mt-3">
                ...
                @if ($edit)
                    <a href="{{ route('affiliate.portal.prospects') }}" class="ml-2 text-sm text-slate-500 hover:underline">Cancel</a>
                @endif
            </div>
        </form>
    </div>
    ...
    <div class="mb-5 rounded-xl border border-slate-200 bg-white p-5">
        ...
        @if ($mine->isEmpty())
            <p class="text-sm text-slate-400">You haven't claimed any prospects yet.</p>
        @else
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead><tr class="text-left text-slate-500"><th class="py-2">Company</th><th>Region</th><th>Status</th><th>Locked until</th><th>Actions</th></tr></thead>
                    <tbody>
                    @foreach ($mine as $p)
                        ...
                        <tr class="border-t border-slate-100">
                            <td class="py-2">
                                ...
                                @if ($p->contact_name)<div class="text-xs text-slate-400">{{ $p->contact_name }}</div>@endif
                            </td>
                            <td class="text-slate-500">{{ trim(($p->city ?? '') . ' ' . ($p->state_province ?? '')) ?: '—' }}</td>
                            <td><x-affiliate-badge :status="$p->status" /></td>
                            <td class="text-slate-500">{{ $active ? $p->claim_expires_at->format('d/m/Y') : '—' }}</td>
                            <td class="whitespace-nowrap">
                                @if ($p->status !== 'won')
                                    <a href="{{ route('affiliate.portal.prospects', ['edit' => $p->id]) }}"
                                       class="rounded border border-slate-300 px-2 py-1 text-xs hover:bg-slate-50">Edit</a>
                                @endif
                                @if ($active)
                                    <form method="POST" action="{{ route('affiliate.portal.prospects.renew', $p) }}" class="inline">@csrf
                                        <button class="rounded border border-slate-300 px-2 py-1 text-xs hover:bg-slate-50">Extend</button>
                                    </form>
                                    <form method="POST" action="{{ route('affiliate.portal.prospects.release', $p) }}" class="inline"
                                        ...
                                        <button class="rounded border border-slate-300 px-2 py-1 text-xs hover:bg-slate-50">Release</button>
                                    </form>
                                @endif
                            </td>
                        </tr>
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-5">
        ...
        <form method="GET" class="mb-3 flex gap-2">
            <input name="q" value="{{ request('q') }}" placeholder="Filter by city / region / industry"
                   class="max-w-xs rounded-lg border-slate-300 text-sm focus:border-orange-500 focus:ring-orange-500">
            <button class="rounded-lg border border-slate-300 px-3 text-sm hover:bg-slate-50">Filter</button>
        </form>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead><tr class="text-left text-slate-500"><th class="py-2">Company</th><th>Region</th><th>Industry</th><th>Stage</th><th>Locked until</th></tr></thead>
                <tbody>
                @forelse ($board as $b)
                    <tr class="border-t border-slate-100 {{ $b->is_mine ? 'bg-orange-50' : '' }}">
                        <td class="py-2">{{ $b->masked_name }} @if($b->is_mine)<span class="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-800">you</span>@endif</td>
                        <td class="text-slate-500">{{ $b->region }}</td>
                        <td class="text-slate-500">{{ $b->industry ?? '—' }}</td>
                        <td><x-affiliate-badge :status="$b->status" /></td>
                        <td class="text-slate-500">{{ \Carbon\Carbon::parse($b->claim_expires_at)->format('d/m/Y') }}</td>
                    </tr>
                @empty
                    <tr><td colspan="5" class="py-4 text-center text-slate-400">No active claims right now — the field is open.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>

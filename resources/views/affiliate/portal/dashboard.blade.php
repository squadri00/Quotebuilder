...
<x-affiliate-layout title="Dashboard" active="dashboard">
    ...
    <div class="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        @foreach ([
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
                <div class="text-[11px] uppercase tracking-wide text-slate-400 dark:text-slate-500">{{ $tile[0] }}</div>
                <div class="mt-1 text-2xl font-bold">{{ $tile[1] }}</div>
                @isset($tile[2])<div class="text-xs text-slate-400 dark:text-slate-500">{{ $tile[2] }}</div>@endisset
            </div>
        @endforeach
        <div class="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
            <div class="text-[11px] uppercase tracking-wide text-slate-400 dark:text-slate-500">Approved &amp; unpaid</div>
            <div class="mt-1 text-2xl font-bold text-orange-600">{{ $m($stats['commission_unpaid']) }}</div>
            <div class="text-xs text-slate-400 dark:text-slate-500">{{ $m($stats['commission_pending']) }} pending approval</div>
        </div>
    </div>
    ...
    <div class="mb-5 rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
        ...
        <div class="flex flex-wrap items-center gap-2 rounded-lg bg-slate-100 p-3 dark:bg-slate-800 font-mono text-sm break-all">
    ...
    @if ($eligible->isNotEmpty())
        <div class="mb-5 rounded-xl border border-orange-300 bg-white p-5 dark:border-orange-900/60 dark:bg-slate-900">
            <h2 class="mb-2 text-sm font-semibold">Statement ready</h2>
            <table class="w-full text-sm">
                <thead><tr class="text-left text-slate-500 dark:text-slate-400"><th class="py-1">Month</th><th>Lines</th><th>Amount</th><th></th></tr></thead>
                <tbody>
    ...
    <div class="grid gap-5 lg:grid-cols-2">
        <div class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
            ...
            @forelse ($recentReferrals as $r)
                @if ($loop->first)
                    <table class="w-full text-sm"><thead><tr class="text-left text-slate-500 dark:text-slate-400"><th class="py-1">Business</th><th>Plan</th><th>Status</th></tr></thead><tbody>
                @endif
                <tr class="border-t border-slate-100 dark:border-slate-800/60">
                ...
                @if ($loop->last)</tbody></table>
                    <a href="{{ route('affiliate.portal.referrals') }}" class="mt-2 inline-block text-xs text-slate-500 dark:text-slate-400 hover:underline">View all →</a>
                @endif
            @empty
                <p class="text-sm text-slate-400 dark:text-slate-500">No referrals yet — share your link to get started.</p>
            @endforelse
        </div>
        <div class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
            ...
            @forelse ($recentCommissions as $c)
                @if ($loop->first)
                    <table class="w-full text-sm"><thead><tr class="text-left text-slate-500 dark:text-slate-400"><th class="py-1">Period</th><th>Business</th><th>Amount</th><th>Status</th></tr></thead><tbody>
                @endif
                <tr class="border-t border-slate-100 dark:border-slate-800/60">
                    <td class="py-2">{{ sprintf('%04d-%02d', $c->period_year, $c->period_month) }}</td>
                    <td class="text-slate-500 dark:text-slate-400">{{ $c->business?->name ?? $c->business_name_snapshot }}</td>
                    <td class="font-semibold">{{ $m($c->commission_amount) }}</td>
                    <td><x-affiliate-badge :status="$c->status" /></td>
                </tr>
                @if ($loop->last)</tbody></table>
                    <a href="{{ route('affiliate.portal.commissions') }}" class="mt-2 inline-block text-xs text-slate-500 dark:text-slate-400 hover:underline">View ledger →</a>
                @endif
            @empty
                <p class="text-sm text-slate-400 dark:text-slate-500">Nothing accrued yet.</p>
            @endforelse
        </div>
    </div>
</x-affiliate-layout>

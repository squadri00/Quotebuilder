...
<x-affiliate-layout title="My referrals" active="referrals">
    <div class="rounded-xl border border-slate-200 bg-white p-5">
        ...
        @if ($rows->isEmpty())
            <p class="text-sm text-slate-400">No referrals yet. Your link:
                    ...
                    @foreach ($rows as $r)
                        <tr class="border-t border-slate-100">
                            <td class="py-2">
                                ...
                                <div class="text-xs text-slate-400">since {{ $r->business?->created_at?->format('d/m/Y') }}</div>
                            </td>
                            ...
                            <td class="text-slate-500">{{ $r->last_payment ? \Carbon\Carbon::parse($r->last_payment)->format('d/m/Y') : '—' }}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>

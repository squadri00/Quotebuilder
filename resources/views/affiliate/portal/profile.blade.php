<x-affiliate-layout title="Profile" active="profile">
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                @foreach ([
                    ...
                    <div>
                        ...
                        <input name="{{ $n }}" value="{{ $v($n) }}" class="w-full rounded-lg border-slate-300 text-sm">
                    </div>
                @endforeach
                <div>
                    <label class="mb-1 block text-xs font-medium">Email (contact support to change)</label>
                    <input value="{{ $partner->email }}" disabled class="w-full rounded-lg border-slate-200 bg-slate-50 text-sm text-slate-400">
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <input name="address" value="{{ $v('address') }}" class="w-full rounded-lg border-slate-300 text-sm">
                </div>
                <div class="sm:col-span-2">
                    <label class="mb-1 block text-xs font-medium">Payout details</label>
                    <input name="payout_details" value="{{ $v('payout_details') }}" class="w-full rounded-lg border-slate-300 text-sm">
                </div>
            </div>
    ...
    <div class="mb-5 rounded-xl border border-slate-200 bg-white p-5">
        ...
        <p class="text-sm">
            Code: <span class="rounded bg-orange-100 px-2 py-0.5 font-mono text-xs font-semibold text-orange-700">{{ $partner->partner_code }}</span> ·
            Rate: <strong>{{ rtrim(rtrim(number_format((float) $partner->commission_rate, 3), '0'), '.') }}%</strong> ·
            Joined {{ $partner->created_at->format('d/m/Y') }}
        </p>
        <div class="mt-2 rounded-lg bg-slate-100 p-3 font-mono text-sm break-all">{{ $partner->referralUrl() }}</div>
    </div>
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-5">
        ...
        <form method="POST" action="{{ route('affiliate.portal.profile.password') }}" class="max-w-sm space-y-3">
            ...
            <div>
                ...
                <input type="password" name="current" required class="w-full rounded-lg border-slate-300 text-sm">
                @error('current') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>
            <div>
            ...
            <button class="rounded-lg border border-slate-300 px-4 py-2 text-sm hover:bg-slate-50">Update password</button>
        </form>
    </div>
</x-affiliate-layout>

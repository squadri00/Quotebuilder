...
<x-affiliate-layout title="Commissions" active="commissions">
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-5">
        ...
        @if ($rows->isEmpty())
            <p class="text-sm text-slate-400">Nothing here yet.</p>
        @else
                ...
                <table class="w-full text-sm">
                    ...
                    @foreach ($rows as $c)
                        <tr class="border-t border-slate-100">
                            ...
                            <td class="text-slate-500">{{ $c->kind }}</td>
                            <td>{{ $c->kind === 'commission' ? $m($c->base_amount) : '—' }}</td>
                            <td>{{ $c->kind === 'commission' ? rtrim(rtrim(number_format((float) $c->rate, 3), '0'), '.') . '%' : '—' }}</td>
                            <td class="font-semibold {{ $c->commission_amount < 0 ? 'text-red-600' : '' }}">{{ $m($c->commission_amount) }}</td>
                            <td><x-affiliate-badge :status="$c->status" /></td>
                            <td class="text-slate-500">{{ $c->payout?->payout_number ?? '—' }}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>

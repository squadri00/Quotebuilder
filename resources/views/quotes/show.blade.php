    ...
    <div class="max-w-lg">
        <x-card>
            <div class="text-center">
                ...
                @else
                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300">
                        Public Quote
                    </span>
                @endif
                ...
                <div class="mt-2 text-sm text-gray-700 dark:text-gray-300">
                    ...
                    @if (! empty($quote->customerContact()['phone']))
                        <p>{{ $quote->customerContact()['phone'] }}</p>
                    @endif
                </div>
                ...
                @if (count($quote->meta['selections'] ?? []))
                    <div class="mt-6 text-left border-t border-gray-200 dark:border-gray-700 pt-4">
                        ...
                        <ul class="space-y-1 text-sm">
                            @foreach ($quote->meta['selections'] as $item)
                    ...
                    <div class="mt-6 text-left border-t border-gray-200 dark:border-gray-700 pt-4">
                        <ul class="space-y-1 text-sm">
                            ...
                            <li class="flex justify-between gap-4 font-semibold text-gray-900 dark:text-gray-100 border-t border-gray-100 dark:border-gray-700 pt-1 mt-1">
                                <span>Total</span>
                                <span class="shrink-0">${{ number_format($quote->final_price, 2) }}</span>
                            </li>
                        </ul>
                    </div>
                @endif
                ...
                <div class="mt-4 grid grid-cols-2 gap-2">
                    ...
                    @if ($pdfEnabled)
                        <a href="{{ route('quote.pdf', $quote) }}" class="inline-flex items-center justify-center gap-2 w-full px-4 py-2.5 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg font-semibold text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600 {{ ! $emailEnabled ? 'col-span-2' : '' }}">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4 w-4">
                ...
                <div class="mt-4 flex items-center justify-center gap-4 text-sm">
                    <a href="{{ route('quotes.index') }}" class="font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">&larr; Back to Quotes</a>
                </div>
            </div>
        </x-card>
    </div>

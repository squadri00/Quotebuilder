<x-superadmin-layout :title="'Demo Quote — '.$product->name">
    <x-slot name="header">
        <div>
            <p class="text-xs text-gray-500 dark:text-gray-400">
                <a href="{{ route('superadmin.quotes.create') }}" class="hover:text-gray-700 dark:hover:text-gray-200">Demo Quote</a> / {{ $product->name }}
            </p>
            <h2 class="font-bold text-xl text-gray-900 dark:text-gray-100">{{ $product->name }}</h2>
        </div>
    </x-slot>
            ...
            <div class="flex-1 order-1 md:order-2">
                ...
                <x-card>
                    ...
                    <div x-show="currentQuestionId === 'DONE'" x-cloak class="text-center py-6">
                        <svg class="animate-spin h-6 w-6 mx-auto text-indigo-600 dark:text-indigo-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                        </svg>
                        <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">Taking you to Review&hellip;</p>

                        <form method="POST" action="{{ route('superadmin.quotes.create.review', $product) }}" class="mt-4" x-ref="doneForm">
                            @csrf

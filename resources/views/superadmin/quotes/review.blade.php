<x-superadmin-layout title="Review Demo Quote">
    <x-slot name="header">
        <div>
            <p class="text-xs text-gray-500 dark:text-gray-400">
                <a href="{{ route('superadmin.quotes.create') }}" class="hover:text-gray-700 dark:hover:text-gray-200">Demo Quote</a> / Review
            </p>
            ...
            <div class="text-center">
                ...
                <form method="POST" id="quote-review-form" class="mt-6 text-left space-y-4">
                    ...
                    <div class="rounded-lg border border-dashed border-gray-300 dark:border-gray-600 p-4">
                        <x-input-label value="Demo Business Details" />
                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">
                            Optional — what should this one quote look like it's from? Nothing here is saved to any real business; leave any of these blank to just show "{{ $business->name }}".
                        </p>
                        <div class="space-y-3">
                            <x-text-input name="demo_business_name" type="text" class="block w-full" placeholder="Business Name" />
                            <x-text-input name="demo_business_address" type="text" class="block w-full" placeholder="Address" />
                            <div class="grid grid-cols-2 gap-3">
                                <x-text-input name="demo_business_phone" type="text" class="block w-full" placeholder="Phone Number" />
                                <x-text-input name="demo_business_email" type="email" class="block w-full" placeholder="Email Address" />
                            </div>
                        </div>
                        <x-input-error :messages="$errors->get('demo_business_email')" class="mt-2" />
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex rounded-lg shadow-sm">
                            <button type="button" @click="customerMode = 'new'; clearCustomer()"
                                ...
                                :class="customerMode === 'new' ? 'bg-indigo-600 text-white border-transparent' : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600'">
                                New Customer
                            </button>
                            <button type="button" @click="customerMode = 'existing'"
                                ...
                                :class="customerMode === 'existing' ? 'bg-indigo-600 text-white border-transparent' : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600'">
                                Existing Customer
                            </button>
                        </div>
                    </div>
                    ...
                    <template x-if="customerMode === 'existing'">
                        <div class="relative">
                            <x-input-label value="Search Past Demo Customers" />
                            ...
                            <p class="mt-2 text-xs text-gray-500 dark:text-gray-400" x-show="selectedCustomer">
                                Selected — <button type="button" class="text-indigo-600 dark:text-indigo-400 font-medium" @click="clearCustomer()">change</button>
                            </p>
                    ...
                    <div class="space-y-2">
                        <button type="submit" formaction="{{ route('superadmin.quotes.create.save', $product) }}"
                            class="inline-flex items-center justify-center gap-2 w-full px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 rounded-lg font-semibold text-sm text-white transition">
                            Save Quote
                        </button>
                        <button type="submit" formaction="{{ route('superadmin.quotes.create.email', $product) }}"
                        ...
                        <button type="submit" formaction="{{ route('superadmin.quotes.create.pdf', $product) }}"
                            class="inline-flex items-center justify-center gap-2 w-full px-4 py-2.5 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg font-semibold text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                            Save &amp; Download PDF
                        </button>
                    </div>
                ...
                <div class="mt-4 flex items-center justify-center gap-4 text-sm">
                    <a href="{{ route('superadmin.quotes.create.show', $product) }}" class="font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">&larr; Back to edit</a>
                </div>
            </div>
        </x-card>
        </div>

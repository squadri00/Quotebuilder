...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-display antialiased bg-white text-gray-700">
            ...
            <div class="flex flex-col max-w-6xl mx-auto px-6 md:items-center md:justify-between md:flex-row">
                <div class="flex flex-row items-center justify-between py-6">
                    <a href="{{ url('/') }}" class="relative z-50 flex items-center shrink-0">
                        <img src="{{ asset('images/quotaire/logo-for-white.png') }}" alt="{{ config('app.name', 'Quotaire') }}" class="h-8 w-auto">
                    </a>

                    <button class="rounded-lg md:hidden focus:outline-none focus:shadow-outline" @click="open = !open" aria-label="Toggle navigation">
                        <svg fill="currentColor" viewBox="0 0 20 20" class="w-6 h-6 text-gray-700">
                ...
                <nav :class="{ 'transform md:transform-none': !open, 'h-full': open }" class="h-0 md:h-auto overflow-hidden flex flex-col flex-grow md:items-center pb-4 md:pb-0 md:overflow-visible md:flex md:justify-end md:flex-row origin-top duration-300 scale-y-0 md:scale-y-100">
                    <a href="{{ url('/') }}" class="px-4 py-2 mt-2 text-base font-semibold {{ request()->is('/') ? 'text-green-600' : 'text-gray-900' }} rounded-lg md:ml-4 hover:text-green-600 focus:outline-none focus:shadow-outline">Home</a>
                    <a href="{{ route('features') }}" class="px-4 py-2 mt-2 text-base font-semibold {{ request()->routeIs('features') ? 'text-green-600' : 'text-gray-900' }} rounded-lg md:ml-4 hover:text-green-600 focus:outline-none focus:shadow-outline">Features</a>
                    <a href="{{ route('pricing') }}" class="px-4 py-2 mt-2 text-base font-semibold {{ request()->routeIs('pricing') ? 'text-green-600' : 'text-gray-900' }} rounded-lg md:ml-4 hover:text-green-600 focus:outline-none focus:shadow-outline">Pricing</a>
                    <a href="{{ route('demo') }}" class="px-4 py-2 mt-2 text-base font-semibold {{ request()->routeIs('demo') ? 'text-green-600' : 'text-gray-900' }} rounded-lg md:ml-4 hover:text-green-600 focus:outline-none focus:shadow-outline">Demo</a>
                    <a href="{{ route('contact') }}" class="px-4 py-2 mt-2 text-base font-semibold {{ request()->routeIs('contact') ? 'text-green-600' : 'text-gray-900' }} rounded-lg md:ml-4 hover:text-green-600 focus:outline-none focus:shadow-outline">Contact</a>
                    ...
                    @else
                        <a href="{{ route('login') }}" class="px-10 py-3 mt-2 text-sm text-center font-semibold bg-gray-100 text-gray-800 rounded-full md:ml-4 hover:bg-gray-200 transition">Login</a>

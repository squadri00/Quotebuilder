<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between w-full">
            ...
            <x-primary-button onclick="window.location='{{ route('rules.create') }}'">New Rule</x-primary-button>
        </div>
    </x-slot>
        ...
        <div class="space-y-4">
            @foreach ($rules as $item)
                <x-card class="flex items-center justify-between">
                    ...
                    <a href="{{ route('rules.edit', $item['rule']) }}" class="text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900">Edit</a>
                </x-card>
            @endforeach
        </div>
    @endif
